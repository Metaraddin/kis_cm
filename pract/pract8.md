...
Задание:
Напишите makefile для python проекта.
В скрипте должно присутствовать:

* Переменная `VENV` - директория виртуального окружения
* Переменная `PYTHON` - путь к исполняемому файлу python виртуального окружения, содержащий переменную `VENV`
* Переменная `PIP` - путь к исполняемому файлу pip виртуального окружения, содержащий переменную `VENV`

* Цель `run`, которая должна запускать файл `main.py` в виртуальном окружении, а в качестве зависимости иметь файл `$(VENV)/bin/activate` (`$(VENV)/Script/activate` для Windows)
* Цель файл `$(VENV)/bin/activate`, которая должна создавать виртуальное окружение и устанавливать в него библиотеки из файла `requirements.txt`
* Цель `clean`, которая должна удалять директорию `__pycache__` и директорию виртуальное окружение

*Прочитать про работу с venv можно в практической работе №4*

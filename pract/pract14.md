...
1. Третий столбец, содержащий `>>` - это метка, которая обозначает возможный переход `JUMP` от более ранней инструкции с данной.
...
По адресу `6` расположена инструкция `LOAD_CONST`
...
На данном проверке у нас производится проверка условия цикла, для чего была загружена переменная `x` при помощи инструкции `LOAD_FAST` и переход по адресу 6 в случае `true` у условия цикла (метку о возможном переходе по данному адресу мы уже видели).
